...
def g_feat(args, model):
    ...
    for idx in tqdm(range(count)):
        ...
        cov_in = model.module.layer_map(cov_cat).squeeze(0).squeeze(0)
        sam_tmp = np.random.multivariate_normal(mean=model.module.mem_feat[idx,:].cpu().numpy(), cov=cov_in.detach().cpu().numpy(), size=30)
